...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 9 — LAYER 2: DISRUPTION EXTRACTION ENGINE
# ══════════════════════════════════════════════════════════════════════════════

DISRUPTION_LEXICON = {
    "strait_of_hormuz": {
        "keywords": ["strait of hormuz", "hormuz strait"],
        "impacts": ["oil exports", "lng exports", "middle east shipping", "persian gulf"],
        "severity": "critical"
    },
    "red_sea": {
        "keywords": ["red sea", "houthi"],
        "impacts": ["suez canal", "asia-europe trade", "container shipping", "vessel delays"],
        "severity": "high"
    },
    "suez_canal": {
        "keywords": ["suez canal", "suez passage"],
        "impacts": ["asia-europe route", "mediterranean trade", "container shipping"],
        "severity": "high"
    },
    "taiwan_strait": {
        "keywords": ["taiwan strait", "taiwan crisis", "cross-strait"],
        "impacts": ["semiconductors", "electronics", "chip exports", "taiwan"],
        "severity": "critical"
    },
    "port_strike": {
        "keywords": ["port strike", "port closure", "dockworkers strike", "longshoreman"],
        "impacts": ["shipping delays", "container backlog", "logistics disruption"],
        "severity": "high"
    },
    "export_ban": {
        "keywords": ["export ban", "import restriction", "embargo"],
        "impacts": ["commodity shortage", "supplier disruption", "supply shortage"],
        "severity": "high"
    },
    "geopolitical_crisis": {
        "keywords": ["sanctions", "trade war", "tariff", "conflict"],
        "impacts": ["supply chain disruption", "trade delays", "cost increases"],
        "severity": "high"
    },
    "natural_disaster": {
        "keywords": ["earthquake", "flood", "typhoon", "cyclone", "wildfire", "hurricane"],
        "impacts": ["facility disruption", "logistics disruption", "infrastructure damage"],
        "severity": "high"
    },
}

def extract_disruption_metadata(article: dict) -> dict:
    """
    Layer 2: Extract structured disruption metadata from article.
    Returns: {event_type, locations[], commodities[], severity}
    """
    text = (article.get("title", "") + " " + article.get("description", "")).lower()
    ...
    # Match against disruption lexicon
    for lex_key, lex_entry in DISRUPTION_LEXICON.items():
        for keyword in lex_entry.get("keywords", []):
            if keyword in text:
                extracted["event_type"] = lex_key
                extracted["severity"] = lex_entry.get("severity", "medium")
                extracted["disruption_keywords"].extend(lex_entry.get("impacts", []))
                break
    
    # Location extraction (basic — match known supply chain hubs)
    locations = {
        "shanghai": ["shanghai", "china", "port of shanghai"],
    ...
    for loc_name, loc_keywords in locations.items():
        if any(kw in text for kw in loc_keywords):
            extracted["locations"].append(loc_name)
    
    # Commodity extraction
    commodities = {
        "semiconductors": ["chip", "semiconductor", "wafer", "foundry"],
        "lithium": ["lithium", "battery", "ev battery"],
        "oil": ["crude oil", "petroleum", "oil"],
        "lng": ["lng", "natural gas", "liquefied gas"],
        "grain": ["wheat", "grain", "corn", "soybeans"],
    }
    
    for commodity_name, commodity_keywords in commodities.items():
        if any(kw in text for kw in commodity_keywords):
            extracted["commodities"].append(commodity_name)
    
    return extracted


# ══════════════════════════════════════════════════════════════════════════════
# SECTION 9A — LAYER 3: SUPPLY CHAIN DEPENDENCY MAPPER
# ══════════════════════════════════════════════════════════════════════════════

def build_entity_dependencies(entity_label: str, entity_fields: dict, entity_type: str) -> dict:
    """
    Layer 3: Build supply chain dependency map for an entity.
    
    Maps entity to: geographic dependencies, commodity flows, routing dependencies, etc.
    """
    dependencies = {
        "geographic": [],
        "commodities": [],
    ...
    location = _clean(entity_fields.get("location", ""))
    if location:
        dependencies["geographic"].append(location.lower())
    
    # Material/commodity dependencies
    if entity_type == "material":
        commodity = _clean(entity_fields.get("commodity", ""))
        region = _clean(entity_fields.get("region", ""))
        if commodity:
            dependencies["commodities"].append(commodity.lower())
        if region:
            dependencies["geographic"].append(region.lower())
    ...
    if entity_type == "logistics":
        port = _clean(entity_fields.get("port", ""))
        route = _clean(entity_fields.get("route", ""))
        if port:
            dependencies["routes"].append(port.lower())
        if route:
            dependencies["routes"].append(route.lower())
    
    return dependencies


# ══════════════════════════════════════════════════════════════════════════════
# SECTION 9B — LAYER 4: IMPACT INFERENCE ENGINE
# ══════════════════════════════════════════════════════════════════════════════

def infer_entity_impact(article: dict, disruption_metadata: dict,
                       entity_label: str, entity_dependencies: dict,
                       entity_type: str) -> tuple[bool, str, float]:
    """
    Layer 4: Determine if article impacts entity through dependency analysis.
    
    Returns: (is_impacted, reason, confidence_score)
    
    Checks:
      1. Direct mention match (legacy system still works)
      2. Dependency overlap (new inference)
      3. Disruption-commodity alignment
      4. Route-geography alignment
    """
    text = (article.get("title", "") + " " + article.get("description", "")).lower()
    confidence = 0.0
    reasons = []
    
    # Check 1: Direct disruption keyword mention
    if article.get("impact_level") in ["CRITICAL", "HIGH", "MEDIUM"]:
        confidence += 0.3
        reasons.append("Direct disruption signal in article")
    
    # Check 2: Dependency overlap analysis
    disruption_locs = [loc.lower() for loc in disruption_metadata.get("locations", [])]
    entity_geos = entity_dependencies.get("geographic", [])
    
    for loc in disruption_locs:
        for entity_geo in entity_geos:
            if loc in entity_geo or entity_geo in loc:
                confidence += 0.25
                reasons.append(f"Geographic dependency overlap: {entity_geo}")
                break
    
    # Check 3: Commodity-disruption alignment
    disruption_commodities = disruption_metadata.get("commodities", [])
    entity_commodities = entity_dependencies.get("commodities", [])
    
    for commodity in disruption_commodities:
        for entity_commodity in entity_commodities:
            if commodity in entity_commodity or entity_commodity in commodity:
                confidence += 0.30
                reasons.append(f"Commodity impact: {entity_commodity}")
                break
    
    # Check 4: Route disruption matching
    if disruption_metadata.get("event_type") in ["red_sea", "strait_of_hormuz", "suez_canal", "taiwan_strait"]:
        entity_routes = entity_dependencies.get("routes", [])
        for route in entity_routes:
            if disruption_metadata["event_type"].replace("_", " ") in route or route in text:
                confidence += 0.35
                reasons.append(f"Critical route disruption: {disruption_metadata['event_type']}")
                break
    
    # Check 5: Broad supply chain disruption keywords
    disruption_keywords = disruption_metadata.get("disruption_keywords", [])
    if any(kw in text for kw in disruption_keywords):
        confidence += 0.20
        reasons.append("Related supply chain disruption detected")
    
    # Normalize confidence to 0-1 range
    confidence = min(confidence, 0.99)
    
    is_impacted = confidence >= 0.25  # threshold for relevance
    
    return is_impacted, "; ".join(reasons) if reasons else "Potential indirect impact", confidence


# ══════════════════════════════════════════════════════════════════════════════
# SECTION 9C — LAYER 5: IMPACT ENRICHMENT
# ══════════════════════════════════════════════════════════════════════════════

def enrich_article_with_intelligence(article: dict, entity_impacts: list) -> dict:
    """
    Layer 5: Enrich article with multi-layer intelligence data.
    
    Adds to article:
      - affected_entities: list of entities impacted
      - impact_reasons: list of impact reasons
      - confidence: average confidence score
    """
    if not entity_impacts:
        article["affected_entities"] = []
        article["impact_reasons"] = []
        article["confidence"] = 0.0
        return article
    
    # Filter impacts that actually affected entities
    real_impacts = [ei for ei in entity_impacts if ei.get("is_impacted")]
    
    article["affected_entities"] = [ei["entity"] for ei in real_impacts]
    article["impact_reasons"] = list(set([
        reason for ei in real_impacts
        for reason in ei.get("reason", "").split("; ")
        if reason
    ]))
    
    article["confidence"] = round(
        sum(ei.get("confidence", 0) for ei in real_impacts) / max(len(real_impacts), 1),
        2
    )
...
def analyze():
    ...
    try:
        ...
        # ── NEW: LAYER 2-5 INTELLIGENCE ENRICHMENT ────────────────────────────
        # Build entity-to-fields mapping for dependency analysis
        entity_fields_map = {}
        for i, s in enumerate(tier1):
            label = f"Supplier: {_clean(s.get('name','')) or _clean(s.get('material','')) or f'#{i+1}'}"
            entity_fields_map[label] = {"fields": s, "type": "supplier"}
        for i, m in enumerate(raw_mats):
            label = f"Material: {_clean(m.get('commodity','')) or f'#{i+1}'}"
            entity_fields_map[label] = {"fields": m, "type": "material"}
        for i, n in enumerate(log_nodes):
            label = f"Logistics: {_clean(n.get('port','')) or _clean(n.get('carrier','')) or _clean(n.get('route','')) or f'#{i+1}'}"
            entity_fields_map[label] = {"fields": n, "type": "logistics"}
        for i, f in enumerate(own_facs):
            label = f"Facility: {_clean(f.get('name','')) or _clean(f.get('location','')) or f'#{i+1}'}"
            entity_fields_map[label] = {"fields": f, "type": "facility"}
        
        # Enrich each article with multi-layer intelligence
        for article in final:
            # Layer 2: Extract disruption metadata
            disruption_meta = extract_disruption_metadata(article)
            article["_disruption_meta"] = disruption_meta  # store for internal use
            
            # Layer 3 & 4: Build dependencies and infer impact for each entity
            entity_impacts = []
            for entity_label, entity_info in entity_fields_map.items():
                # Layer 3: Build entity dependencies
                dependencies = build_entity_dependencies(
                    entity_label,
                    entity_info["fields"],
                    entity_info["type"]
                )
                
                # Layer 4: Infer impact
                is_impacted, reason, confidence = infer_entity_impact(
                    article,
                    disruption_meta,
                    entity_label,
                    dependencies,
            ...
            # Layer 5: Enrich article with intelligence
            article = enrich_article_with_intelligence(article, entity_impacts)
